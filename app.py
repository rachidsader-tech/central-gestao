...
# Carrega o núcleo já estável do sistema. A interface V4 não depende mais de HTML comprimido.
...
app.view_functions['index']=_v4_index

def _v4_health():
    return jsonify({'ok':True,'service':'transformacao-kaz','version':'4.0'})
app.view_functions['health']=_v4_health
...
app.view_functions['manual']=_v4_manual

@login_required
def _v4_project_action(project_id):
    require_csrf()
    ...
    if action=='status':
        ...
        p['lastUpdate']=now_iso()
        p.setdefault('history',[]).append({'at':now_iso(),'author':u.display_name,'text':'Estrutura inicial dos marcos finalizada e bloqueada para o responsável.'})
        ...
        # A estrutura fica congelada após a revisão inicial do líder.
        # A gestão operacional continua: status, memorando e conclusão.
        ...
        p.setdefault('history',[]).append({'at':now_iso(),'author':u.display_name,'text':'Diretoria reabriu excepcionalmente a edição estrutural dos marcos.'})
...
app.view_functions['project_action']=_v4_project_action

@login_required
def _v4_create_meeting():
    require_csrf()
    body=request.get_json(force=True)
    u=current_user()
...
app.view_functions['create_meeting']=_v4_create_meeting
